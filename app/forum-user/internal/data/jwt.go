...
func (h *handler) CheckSession(ctx context.Context, ssid string) error {
	// 判断缓存中是否存在指定键
	c, err := h.client.Exists(ctx, fmt.Sprintf("forum:user:ssid:%s", ssid)).Result()
	if err != nil {
		return err
	}
	if c != 0 {
...
func (h *handler) ClearToken(ctx context.Context) error {
	...
	if er := h.client.Set(ctx, fmt.Sprintf("forum:user:ssid:%s", uc.Ssid), "", remainingTime).Err(); er != nil {
		return er
	}
	return nil
}
