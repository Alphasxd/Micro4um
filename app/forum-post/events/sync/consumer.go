...
func NewSyncConsumer(client sarama.Client, l *zap.Logger, db *gorm.DB, mongoClient *mongo.Client, postData biz.PostData) *SyncConsumer {
	// 创建MongoDB客户端
	return &SyncConsumer{
		client:      client,
		l:           l,
...
func (r *SyncConsumer) Start(_ context.Context) error {
	...
	go func() {
		for {
			if err := cg.Consume(context.Background(), []string{"forum_binlog_microservices"}, &consumerGroupHandler{r: r}); err != nil {
...
func (r *SyncConsumer) Consume(sess sarama.ConsumerGroupSession, msg *sarama.ConsumerMessage) {
	...
	if e.Database != "forum_microservices" && e.Table != "posts" {
		return
	}
	...
	for _, post := range posts {
		if err := r.handlePost(sess.Context(), post); err != nil {
			r.l.Error("处理帖子失败", zap.Int64("id", post.ID), zap.Error(err))
			return
		}
	}
...
func (r *SyncConsumer) pushOrUpdateMongo(ctx context.Context, post Post) error {
	...
	collection := r.mongoClient.Database("forum_microservices").Collection("posts")
...
func (r *SyncConsumer) deleteMongo(ctx context.Context, post Post) error {
	...
	if result.DeletedCount == 0 {
		r.l.Info("帖子不存在，可能已经被删除", zap.Int64("id", post.ID))
		// 认为这是正常情况，返回 nil 表示没有错误
		return nil
	}

	r.l.Info("帖子已成功删除", zap.Int64("id", post.ID))
	return nil
}
